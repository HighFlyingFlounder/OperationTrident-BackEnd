...
public class Scene
{
	...
	//删除玩家, 可用, 向所有人广播而不是在单个房间内广播??
	//广播协议:PlayerLeave, string id
	...
	//发送列表
	//发送协议:GetList, int count, [string id, int x, int y, int z, int score]*count
	...
	{
		int count = list.Count;
		ProtocolBytes protocol = new ProtocolBytes(); //意义?
		ScenePlayer p = GetScenePlayer(id);
		if (p == null)
			return;
		p.x = x;
